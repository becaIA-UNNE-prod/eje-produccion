            ...
            for fecha in fechas_del_mes:
                ...
                if os.path.exists(ruta_banda):
                    try:
                        ...
                        print(f"[{mes}] Saltando archivo corrupto: {ruta_banda} ({e})")
                        continue
...
if __name__ == "__main__":
    ...
    tiles = sorted([d for d in os.listdir(BASE_S2) if os.path.isdir(os.path.join(BASE_S2, d))])
    print(f"Procesando {len(tiles)} tiles filtrados...")

    for tile in tiles:
        print(f"\n=== Tile: {tile} ===")
        generar_composiciones_mensuales(tile, BASE_S2, DIR_COMPOSITES, max_workers=2)
